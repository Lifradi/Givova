...
def extrair_dados_seguros(texto_pdf):
    """Extrai os dados do PDF utilizando Regex com âncoras contextuais."""
    dados = {}
    ...
    dados['cnpj_destinatario'] = "00000000000000"
...
def gerar_xml_padrao_sefaz(texto_pdf, nome_arquivo):
    ...
    nfe_proc = ET.Element("nfeProc", attrib={"xmlns": "http://www.portalfiscal.inf.br/nfe", "versao": "4.00"})
    NFe = ET.SubElement(nfe_proc, "NFe", attrib={"xmlns": "http://www.portalfiscal.inf.br/nfe"})
    infNFe = ET.SubElement(NFe, "infNFe", attrib={"Id": f"NFe{chave}", "versao": "4.00"})
    ...
    # Bloco TRANSP (Transporte e Volumes)
    transp = ET.SubElement(infNFe, "transp")
    ET.SubElement(transp, "modFrete").text = "0"

    vol = ET.SubElement(transp, "vol")
    ET.SubElement(vol, "qVol").text = dados['qVol']
    ET.SubElement(vol, "esp").text = dados['esp']
    ET.SubElement(vol, "pesoL").text = dados['pesoL']
    ET.SubElement(vol, "pesoB").text = dados['pesoB']
